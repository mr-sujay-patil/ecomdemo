...
class OrderApiIT extends IntegrationTest {
    ...
    void placeOrderEndToEnd() {
        ...
        rest.postForEntity("/api/cart/items", new AddCartItemRequest(headset.id(), 2), CartResponse.class);

        ResponseEntity<OrderResponse> response = rest.postForEntity("/api/orders", null, OrderResponse.class);
        ...
        ResponseEntity<OrderResponse> fetched =
                rest.getForEntity("/api/orders/" + placed.id(), OrderResponse.class);
    ...
    void insufficientStockRollsEverythingBack() {
        ...
        rest.postForEntity("/api/cart/items", new AddCartItemRequest(plenty.id(), 2), CartResponse.class);
        rest.postForEntity("/api/cart/items", new AddCartItemRequest(scarce.id(), 3), CartResponse.class);

        ResponseEntity<ApiError> response = rest.postForEntity("/api/orders", null, ApiError.class);
    ...
    void theLastUnitIsSoldExactlyOnce() throws Exception {
        ...
        rest.postForEntity("/api/cart/items", new AddCartItemRequest(lastOne.id(), 1), CartResponse.class);
        ...
        Callable<ResponseEntity<String>> checkout = () -> {
            start.await();
            return rest.postForEntity("/api/orders", null, String.class);
        };
        ...
        ResponseEntity<OrderResponse[]> orders = rest.getForEntity("/api/orders", OrderResponse[].class);
