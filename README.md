...
Everything until now happened inside one request: the customer waited while the order was written,
the stock decremented, the cart emptied. This phase is where the system learns to say *"this
happened"* and let somebody else care — still inside the monolith, so the only new thing is the
messaging itself.

```text
POST /api/orders ──commit──> OrderPlacedEvent ──async──> orders.placed ──> notification consumer
                     │                                    (3 partitions)      │
                   201 to the customer, immediately                    notification row + log line
```

### A topic is a log, not a queue

The mental model that makes Kafka click: `orders.placed` is an **append-only log** that keeps its
messages whether or not anyone reads them, and a consumer is a bookmark into it. A classic queue
deletes on delivery, so one consumer's read denies it to everyone else; Kafka's consumers each
track an **offset**, so a second consumer group added in a later phase reads the same messages from
the beginning without the producer knowing it exists.

| Term | What it is here |
|---|---|
| **Topic** | `orders.placed` — one named log |
| **Partition** | Three of them. Ordering is guaranteed *within* a partition and nowhere else |
| **Offset** | Each consumer group's bookmark, committed after the listener returns |
| **Consumer group** | `ecomdemo-notification`. Partitions are split across its members — so **three is the ceiling on useful parallelism** |
| **Key** | The order id. Kafka hashes it to choose a partition, which is how one order's events stay in sequence |

That last row is the whole reason a key exists. It is not an identifier for the message; it is a
routing decision that buys ordering.

### Delivery semantics, stated honestly

**Kafka delivers at least once.** Not "exactly once", whatever a configuration page suggests. A
consumer that writes its row and dies before committing its offset will be handed the same record
again, and so will every consumer in a group after a rebalance. The broker cannot know whether the
work happened, because the work happened in PostgreSQL.

So exactly-once is a property the **consumer** provides:

```java
@Transactional
public boolean handle(OrderPlacedEvent event) {
    if (processedEvents.existsById(event.eventId())) return false;   // seen it
    processedEvents.save(new ProcessedEvent(event.eventId(), ...));  // remember FIRST
    notifications.save(new Notification(...));                        // then do the work
}
```

Three details carry it:

- **The event id comes from the producer**, generated once and travelling with the bytes. A
  redelivery is the same bytes, so it carries the same id. An id generated by the consumer — or a
  database sequence — would differ on every delivery and recognise nothing.
- **It is the PRIMARY KEY** of `processed_event`, not a column checked with a `SELECT`. Two
  consumers racing on the same record both pass a check-then-insert; only one survives a primary
  key.
- **Marker and work share one transaction.** Either half alone is worse than neither: a
  notification without a marker is duplicated on redelivery, a marker without a notification means
  the order is never notified and nothing retries.

### Retries belong on topics, not in the thread

Kafka hands out **partitions**, not messages. A consumer that catches an exception and sleeps holds
its partition — so one bad message with a thirty-second backoff stops everything behind it. That is
head-of-line blocking, and it is how a single malformed order delays every order after it.

`@RetryableTopic` forwards the failure to `orders.placed-retry-0`, commits the original offset, and
moves on:

```text
orders.placed ──fail──> orders.placed-retry-0 ──fail──> orders.placed-retry-1 ──fail──> orders.placed-dlt
   (partition keeps moving)      ~1s                        ~2s                      (nothing consumes it)
```

Two details worth stealing:

- **`SUFFIX_WITH_INDEX_VALUE`.** Spring names retry topics after the *delay* by default. With
  jitter that is a different number every start — the first run here created
  `orders.placed-retry-1031` and `-retry-1661`, and every restart would have made two more, for
  ever. Naming by index keeps them stable.
- **`ErrorHandlingDeserializer`.** A malformed message fails *before* any of your code runs, so
  nothing can catch it, the offset is never committed, and the consumer re-reads the same bytes
  for ever. The wrapper turns that into a failure the container can route to the DLT.
...
```text
   transaction commits  ──────X────────>  publish to Kafka
                          process dies
   order is in the database.  Nothing was published.  Nothing knows.
```

This is the **dual-write problem** — two systems, no shared transaction — and no amount of
retrying fixes it, because nothing is aware the message is missing. It was measured rather than
assumed:

```bash
docker stop ecomdemo-kafka
# place an order -> HTTP 201 in 0.18s, the customer is unaffected
docker exec ecomdemo-db psql -U ecomdemo -d ecomdemo -c \
  "SELECT count(*) FROM orders o WHERE NOT EXISTS (SELECT 1 FROM notification n WHERE n.order_id = o.id);"
# 4  <- orders that came out of the outage un-notified, permanently
```
...
The first version of `OrderEventPublisher` looked asynchronous — `send` returns a future — and
with the broker stopped a checkout returned 201 **after 97 seconds**. Two things were wrong:
`send` blocks *inside the call* until the producer has cluster metadata (`max.block.ms`, sixty
...
```bash
docker compose up -d --build
# Kafka UI: http://localhost:8090 — watch orders.placed fill up, partition by partition

# Place an order, then find its notification
docker exec ecomdemo-db psql -U ecomdemo -d ecomdemo -c \
  "SELECT order_id, recipient, message FROM notification ORDER BY id DESC LIMIT 3;"

# Read the topic yourself, keys included
...
# Send a poison message and watch it land on the DLT while the partition keeps moving
...
# Consumer lag: how far behind the notification group is
